...
pub fn add <const N: usize> (a : &[u64; N], b : &[u64; N], _params :&super::prime_fields::FieldParams<N>) -> [u64; N]
    ...
    {   let mut result: [u64; N]= [0; N];
        ...
        if  (bigger >= 0) | carry {
            get_borrowed(&mut tmp, &mut result, &_params, &mut carry, false);
        }
        result
    }
...
pub fn sub <const N: usize> (a : &[u64; N], b : &[u64; N], _params :&super::prime_fields::FieldParams<N>)-> [u64; N]
    ...
    {   let mut result: [u64; N]= [0; N];
        ...
        let mut tmp:u64;
        for i in 0..N {
            (tmp,carry) = b[i].overflowing_add(borrow as u64);
        ...
        if borrow {
            let mut carry:bool=false;
            for i in 0..N {
                (tmp,borrow) = _params.modulo[i].overflowing_add(carry as u64);
...
pub fn neg <const N: usize> (a : &[u64; N],_params :&super::prime_fields::FieldParams<N>) ->[u64; N]
    ...
    {   let mut result: [u64; N]= [0; N];
        let mut borrow: bool =false;
        for i in 0..N {
            (result[i],borrow) = (&_params.modulo[i]).overflowing_sub((a[i]) + (borrow as u64))
        };
        result
    }
    ...
    // CIOS implementation of montgomery multiplication (https://www.microsoft.com/en-us/research/wp-content/uploads/1998/06/97Acar.pdf)
{
        ...
        for i in 0..N {
            ci = 0;
            for j in 0..N {
                t128 = result[j] as u128 +  (a[j] as u128 * b[i] as u128) + ci;
                (ci, result[j]) = (t128 >> 64 , t128 as u64);
            }
            t128  = (tn as u128) +ci ;
            (tnp1, tn) = (t128 >> 64 , t128 as u64);
            m  = (result[0] as u128 * _params.qprime) & ((1 << 64) - 1);
            ci = (result[0] as u128 + m * _params.modulo[0] as u128) >> 64;
            get_ci(&mut result, &_params, m, &mut ci);
            t128  = (tn as u128) + ci;
            (ci, result[N-1]) = (t128 >> 64 , t128 as u64);
            tn = (ci + tnp1) as u64;
        }
        let mut bigger:i32=0;
        get_bigger(&mut bigger, &result, &_params);
        if (bigger >= 0) | (tn!=0) {
            let mut carry: bool = false;
            let mut tmp:u64 = 0u64;
            get_borrowed(&mut tmp, &mut result, &_params, &mut carry, false)
...
fn get_bigger<const N: usize>(bigger: &mut i32, result: &[u64; N], _params: &super::prime_fields::FieldParams<N>) {
    for i in 0..N {
        *bigger += (((result[i] > _params.modulo[i]) as i32) - ((result[i] < _params.modulo[i]) as i32)) << i
    };
}

fn update_result<const N: usize>(bigger: i32, result: &mut [u64; N], _params: &super::prime_fields::FieldParams<N>) {
    if bigger >= 0 {
        let mut borrow:bool=false;
        for i in 0..N {
            (result[i],borrow) = result[i].overflowing_sub((&_params.modulo[i]) + (borrow as u64))
        }
    }
}

fn get_borrowed<const N: usize>(tmp: &mut u64, result: &mut [u64; N], _params: &super::prime_fields::FieldParams<N>,
                                carry: &mut bool, mut borrow: bool) {
    for i in 0..N {
        (*tmp, *carry)= _params.modulo[i].overflowing_add(borrow as u64);
...
pub fn mul<const N: usize> (a : &[u64; N], b : &[u64; N],_params :&super::prime_fields::FieldParams<N>)-> [u64; N]
        ...
        // Optimized using trick from  https://hackmd.io/@gnark/modular_multiplication#fn1
    {
        if  !_params.optimize_mul {mul_std(&a, &b, _params)}
                ...
                let mut bigger:i32=0;
                get_bigger(&mut bigger, &result, &_params);
                update_result(bigger, &mut result, &_params);
                result
            }
    }

fn get_ci<const N: usize>(result: &mut [u64; N], _params: &super::prime_fields::FieldParams<N>, m: u128, ci: &mut u128) {
    for j in 1..N {
        let t128 = result[j] as u128 + (m * _params.modulo[j] as u128) + *ci;
        (*ci, result[j-1]) = (t128 >> 64 , t128 as u64);
    }
}
...
pub fn sqr <const N: usize> (a : &[u64; N],_params :&super::prime_fields::FieldParams<N>)-> [u64; N]
    ...
    {   if ! _params.optimize_sqr {mul(&a, &a, _params)}
                ...
                let mut carry3    :bool;
                for i in 0..N {
                    t128  = (result[i] as u128) + (a[i] as u128 * a[i] as u128);
                    (ci , result[i]) = (t128 >> 64 , t128 as u64);
                    ai = 0;
                    for j in i+1..N {
                        t128  = a[j] as u128 * a[i] as u128;
                        (t128, carry1)  = t128.overflowing_add(t128);
                        (t128, carry2)  = t128.overflowing_add(ci + result[j] as u128);
                        (t128, carry3)  = t128.overflowing_add((ai) << 64);
                        ai = ((carry1 as u8) + (carry2 as u8) + (carry3 as u8)) as u128;
                        (ci, result[j]) = (t128 >> 64,t128 as u64);
                    }
                    ai = ci;
       ...
       fn r_shift <const N: usize> (result:&mut [u64; N], carry :u64)
        ...
        fn sub_noreduction <const N: usize> (a : &[u64; N],result:&mut [u64; N]) -> bool
            ...
            {   let mut borrow: bool =false;
                ...
                let mut tmp: u64 ;
                for i in 0..N {
                    (tmp,carry) = a[i].overflowing_add(borrow as u64);
                    (result[i],borrow) = result[i].overflowing_sub(tmp);
                    borrow= borrow | carry
                };
                borrow
        ...
        fn perform_while<const N: usize>(u: &mut [u64; N], carry: &mut u64, b: &mut [u64; N], _params: &super::prime_fields::FieldParams<N>) {
            while u[0] & 1 == 0 {
                r_shift(u,0);
                *carry =0;
                if b[0] & 1 == 1 {
        ...
        loop {
            perform_while(&mut u, carry, &mut b, &_params);
            perform_while(&mut v, carry, &mut c, &_params);

            if cmp(&u , &v) == 1 {
                sub_noreduction(&v,&mut u);
                if sub_noreduction(&c,&mut b) {
                    add_p(&mut b,&_params.modulo,carry);
                }
            } else {
                sub_noreduction(&u,&mut v);
                if sub_noreduction(&b,&mut c) {
                    add_p(&mut c,&_params.modulo,carry)
                }
            }
            let mut _null_u =0;
            let mut _null_v =0;
...
pub fn sqrt<const N: usize> (a:&[u64; N],_params :&super::prime_fields::FieldParams<N>) -> Option<[u64; N]> {
    if _params.sqrtid == -1 { // p mod 4 ==3
        let result = pow(a,&_params.modplus1div4,false,_params);
        if equal(&mul(&sqr(&result,_params),&invert(a,_params),_params),&_params.one) {
            Some(result)
        } else {
            None
        }
